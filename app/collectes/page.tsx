...
export default function CollectesPage() {
  ...
  const loadData = async () => {
    try {
      ...
      const collectesData = await collecteService.getAll();
  ...
  const filteredCollectes = collectes.filter(collecte => {
    ...
    const matchSearch = 
      ...
      collecte.agent.nomAgent.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const exportToPDF = () => {
    ...
    // Créer le tableau
    autoTable(doc, {
      head: [['Date', 'Client', 'Numéro Compte', 'Agent', 'Montant', 'Mode de Paiement']],
      body: tableData,
      startY: 42,
      styles: { fontSize: 7 },
    ...
    // Sauvegarder le PDF
    const fileName = `versements_admin_${new Date().toISOString().split('T')[0]}.pdf`;
    doc.save(fileName);
  };
  ...
  const exportToExcel = () => {
    ...
    const excelData: Array<Record<string, string | number>> = filteredCollectes.map(collecte => ({
      ...
      'Agent': collecte.agent.nomAgent + ' (' + collecte.agent.codeAgent + ')',
    ...
    const headerRows: string[][] = [
      ...
      [],
      ['Date', 'Client', 'Numéro Compte', 'Agent', 'Montant (FCFA)', 'Mode de Paiement'],
    ];
    ...
    const fileName = `versements_admin_${new Date().toISOString().split('T')[0]}.xlsx`;
            ...
            <div className="bg-white rounded-lg shadow p-6 mb-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    type="text"
                    placeholder="Rechercher par client, agent, compte ou montant..."
                    value={searchTerm}
