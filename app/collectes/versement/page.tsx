...
import { collecteService, Collecte, CreateVersementRequest } from '@/lib/services/collecteService';
...
export default function VersementPage() {
  ...
  const [formData, setFormData] = useState<CreateVersementRequest>({
    numeroCompte: '',
    montant: 0,
  ...
  const loadCollectes = async () => {
    try {
      setLoading(true);
      const data = await collecteService.getAll();
      setCollectes(data);
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      const created = await collecteService.createVersement(formData);
      setCollectes([created, ...collectes]);
      setShowForm(false);
      setFormData({ numeroCompte: '', montant: 0, modePaiement: 'Espèces', preuvePhoto: '' });
      setCompteInfo(null);
      await loadCollectes();
    } catch (err: any) {
      setError(err.response?.data?.message || err.response?.data || 'Erreur lors de la création du versement');
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const filteredCollectes = collectes.filter((collecte) =>
    collecte.compte.numeroCompte.toLowerCase().includes(searchTerm.toLowerCase()) ||
          ...
          <div className="px-4 py-6 sm:px-0">
            ...
            {showForm && (
              <div className="bg-white rounded-2xl shadow-md p-6 mb-6">
                <h2 className="text-xl font-bold text-gray-800 mb-4">Nouveau Versement</h2>
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div>
                    ...
                    <select
                      required
                      className="block w-full py-3 px-4 border border-gray-300 rounded-xl bg-gray-50 text-gray-900 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-2">
                      URL de la preuve photo (Optionnel)
                    </label>
                    <input
                      type="text"
                      className="block w-full py-3 px-4 border border-gray-300 rounded-xl bg-gray-50 placeholder-gray-400 text-gray-900 focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent"
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 gap-4">
                {filteredCollectes.map((collecte) => (
                  <div key={collecte.idCollecte} className="bg-white rounded-2xl shadow-md p-6 hover:shadow-lg transition-shadow">
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        <div className="flex items-center mb-2">
                          <h3 className="text-lg font-bold text-gray-800 mr-3">
                            Versement #{collecte.idCollecte}
                          </h3>
                          <span className="px-3 py-1 bg-green-100 text-green-700 rounded-lg text-xs font-semibold">
                            {new Date(collecte.dateCollecte).toLocaleDateString('fr-FR')}
                          </span>
                        </div>
                        <div className="space-y-1 mt-3">
                          <p className="text-sm text-gray-600">
                            <span className="font-semibold">Compte:</span> {collecte.compte.numeroCompte}
                          </p>
                          <p className="text-sm text-gray-600">
                            <span className="font-semibold">Commerçant:</span> {collecte.compte.commerçant.nomComplet}
                          </p>
                          <p className="text-lg font-bold text-green-600">
                            Montant: {Number(collecte.montant).toLocaleString('fr-FR')} FCFA
                          </p>
                          <p className="text-sm text-gray-600">
                            <span className="font-semibold">Mode de paiement:</span> {collecte.modePaiement}
                          </p>
                          <p className="text-xs text-gray-400 mt-2">
                            Effectué le {new Date(collecte.dateCollecte).toLocaleString('fr-FR')}
                          </p>
                        </div>
                      </div>
                    </div>
