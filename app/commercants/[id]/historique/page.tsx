...
import { Collecte } from '@/lib/services/collecteService';
import { Retrait } from '@/lib/services/retraitService';
import { compteService, Compte } from '@/lib/services/compteService';
import { jsPDF } from 'jspdf';
import autoTable from 'jspdf-autotable';
import * as XLSX from 'xlsx';

export default function CommercantHistoriquePage() {
  const router = useRouter();
  const params = useParams();
  const commerçantId = params?.id ? parseInt(params.id as string) : null;
  ...
  const [filterType, setFilterType] = useState<'ALL' | 'COLLECTE' | 'RETRAIT'>('ALL');
  ...
  const loadData = async () => {
    if (!commerçantId) return;
    
    try {
      setLoading(true);
      setError('');
      const [commercantData, collectesData, retraitsData, comptesData] = await Promise.all([
        commerçantService.getById(commerçantId),
        commerçantService.getCollectes(commerçantId),
        commerçantService.getRetraits(commerçantId),
        compteService.getByCommercant(commerçantId),
      ]);
  ...
  // Créer une liste combinée de toutes les activités (collectes et retraits)
  type Activity = {
    type: 'COLLECTE' | 'RETRAIT';
    id: number;
    date: Date;
    montant: number;
    data: Collecte | Retrait;
  };
  ...
  const totalCollecte = collectes.reduce((sum, c) => sum + Number(c.montant), 0);
  
  const totalRetrait = retraits.reduce((sum, r) => sum + Number(r.montant), 0);

  // Calculer le solde actuel total (somme de tous les comptes)
  const soldeActuelTotal = comptes.reduce((sum, compte) => sum + Number(compte.soldeActuel), 0);
  ...
  const exportToPDF = () => {
    ...
    // Titre
    doc.setFontSize(18);
    doc.text(`Historique du Client: ${commercant?.nomComplet || ''}`, 14, 22);
    ...
    const tableData = filteredActivities.map(activity => {
      if (activity.type === 'COLLECTE') {
        ...
        return [
          ...
          retrait.utilisateurCaisse?.email || '-',
    ...
    // Créer le tableau
    autoTable(doc, {
      head: [['Date', 'Type', 'Numéro Compte', 'Agent/Caisse', 'Montant', 'Mode de Paiement/Motif']],
      body: tableData,
      startY: 42,
      styles: { fontSize: 7 },
    ...
    // Ajouter les totaux
    const totalVersements = filteredActivities.filter(a => a.type === 'COLLECTE').reduce((sum, a) => sum + a.montant, 0);
    const totalRetraits = filteredActivities.filter(a => a.type === 'RETRAIT').reduce((sum, a) => sum + a.montant, 0);
    const finalY = (doc as any).lastAutoTable?.finalY || 42;
    doc.setFontSize(10);
    doc.text(`Total Versements: ${filteredActivities.filter(a => a.type === 'COLLECTE').length}`, 14, finalY + 10);
    ...
    // Sauvegarder le PDF
    const fileName = `historique_client_${commercant?.nomComplet?.replace(/\s+/g, '_') || 'client'}_${new Date().toISOString().split('T')[0]}.pdf`;
    doc.save(fileName);
  };
  ...
  const exportToExcel = () => {
    ...
    const excelData: Array<Record<string, string | number>> = filteredActivities.map(activity => {
      if (activity.type === 'COLLECTE') {
        ...
        return {
          ...
          'Agent/Caisse': collecte.agent?.nomAgent ? `${collecte.agent.nomAgent} (${collecte.agent.codeAgent})` : '-',
          'Montant (FCFA)': collecte.montant,
          'Mode de Paiement/Motif': collecte.modePaiement,
        };
      } else {
        const retrait = activity.data as Retrait;
        return {
          ...
          'Agent/Caisse': retrait.utilisateurCaisse?.email || '-',
          'Montant (FCFA)': retrait.montant,
          'Mode de Paiement/Motif': retrait.motif || '-',
        };
      }
    });
    ...
    const headerRows: string[][] = [
      [`Historique du Client: ${commercant?.nomComplet || ''}`],
      dateDebut || dateFin ? [`Période: ${dateDebut || 'Début'} - ${dateFin || 'Fin'}`] : [''],
      [`Exporté le: ${new Date().toLocaleDateString('fr-FR')}`],
      [],
      ['Date', 'Type', 'Numéro Compte', 'Agent/Caisse', 'Montant (FCFA)', 'Mode de Paiement/Motif'],
    ];
    ...
    const dataRows: (string | number)[][] = excelData.map(row => [
      ...
      row['Agent/Caisse'] as string,
      row['Montant (FCFA)'] as number,
      row['Mode de Paiement/Motif'] as string,
    ]);
    ...
    const fileName = `historique_client_${commercant?.nomComplet?.replace(/\s+/g, '_') || 'client'}_${new Date().toISOString().split('T')[0]}.xlsx`;
      ...
      <div className="min-h-screen bg-gray-50 flex">
        ...
        <div className="page-with-sidebar">
          ...
          <div className="text-center">
            ...
            <Link href="/commercants" className="text-blue-600 hover:text-blue-800 mt-4 inline-block">
      ...
      <div className="page-with-sidebar">
        ...
        <div className="px-4 py-6 sm:px-0">
            ...
            <div className="bg-gradient-to-r from-green-600 to-green-700 rounded-xl shadow-lg p-6 text-white">
              <div className="flex items-center justify-between mb-2 flex-wrap">
                <div className="flex items-center space-x-3">
                  ...
                  <h1 className="text-3xl font-bold">Historique du Client</h1>
                </div>
              ...
              <div className="flex items-center space-x-4 flex-wrap">
                <div>
                  ...
                  <p className="text-xl font-semibold">{commercant.nomComplet}</p>
                </div>
                {commercant.nomBoutique && (
                  <>
                    <div className="h-8 w-px bg-green-400"></div>
                    <div>
                      <p className="text-green-100 text-sm">Boutique</p>
                      <p className="text-xl font-semibold">{commercant.nomBoutique}</p>
                    </div>
                  </>
                )}
                <div className="h-8 w-px bg-green-400"></div>
                <div>
                  ...
                  <p className="text-xl font-semibold">{commercant.zoneCollecte}</p>
                </div>
                <div className="h-8 w-px bg-green-400"></div>
                <div>
                  <p className="text-green-100 text-sm">Téléphone</p>
                  <p className="text-xl font-semibold">{commercant.telephone}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-gray-900">
                    {soldeActuelTotal.toLocaleString('fr-FR')} FCFA
                  </p>
                </div>
            ...
            {(collectes.length > 0 || retraits.length > 0) && (
              <>
          ...
          {filteredActivities.length === 0 ? (
            <div className="bg-white rounded-lg shadow-md p-12 text-center">
              ...
              <p className="text-gray-400 text-sm mt-2">
                {filterType === 'ALL' 
                  ? "Ce client n'a pas encore d'activité enregistrée"
                  : filterType === 'COLLECTE'
                  ? "Ce client n'a pas encore de versement enregistré"
                  : "Ce client n'a pas encore de retrait enregistré"}
              </p>
            </div>
          ) : (
                    ...
                    <div className="p-6">
                      <div className="flex items-start space-x-4">
                        ...
                        <div className="flex-shrink-0 w-16 h-16 rounded-xl flex items-center justify-center shadow-md bg-blue-100 text-blue-600">
                          {getModePaiementIcon(collecte.modePaiement)}
                        </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                              ...
                              <div>
                                ...
                                <p className="text-sm font-semibold text-gray-800 flex items-center">
                                  <span className="mr-2 text-blue-600">{getModePaiementIcon(collecte.modePaiement)}</span>
                              ...
                              {collecte.agent && (
                                <div>
                                  ...
                                  <p className="text-sm font-semibold text-gray-800">
                                    {collecte.agent.nomAgent} ({collecte.agent.codeAgent})
                                  </p>
                                </div>
                              )}
                            </div>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <div className="bg-red-50 rounded-lg p-4 mb-3 border border-red-100">
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                {retrait.utilisateurCaisse && (
                                  <div>
                                    <p className="text-xs text-gray-600 mb-1">Effectué par</p>
                                    <p className="text-sm font-semibold text-gray-800">
                                      {retrait.utilisateurCaisse.email}
                                    </p>
                                  </div>
